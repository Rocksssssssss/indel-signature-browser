indel_template_type_4_full_figurelabel <- 
  structure(list(IndelType = c("A[Del(C):R1]A", "A[Del(C):R1]G", "A[Del(C):R1]T", "A[Del(C):R2]A", "A[Del(C):R2]G", 
                               ...
                               "G[Ins(T):R9]A", "G[Ins(T):R9]C", "G[Ins(T):R9]G", 
...
indel_template_type_4_full_figurelabel$Indel3[grepl("Ins",indel_template_type_4_full_figurelabel$IndelType)] <- "Ins2"
indel_template_type_4_full_figurelabel$Indel3[grepl("Del",indel_template_type_4_full_figurelabel$IndelType)] <- "Del2"

indel_template_type_4_full_figurelabel$Indel3[grepl("Del\\(C\\)|Del\\(T\\)",indel_template_type_4_full_figurelabel$IndelType)] <- "Del1"
indel_template_type_4_full_figurelabel$Indel3[grepl("Ins\\(C\\)|Ins\\(T\\)",indel_template_type_4_full_figurelabel$IndelType)] <- "Ins1"

indel_template_type_4_full_figurelabel$Indel3[grepl("Complex",indel_template_type_4_full_figurelabel$IndelType)] <- "Complex"
indel_template_type_4_full_figurelabel$Indel3[451:471] <- "DelMH"
# Adjust the number of rows and classes as needed
indel_template_type_4_full_figurelabel <- indel_template_type_4_full_figurelabel[1:476, ]

plotID476toPdf <- function (muts_basis, colnum, text_size = 3, h = NULL, w = NULL, 
                            print_Xlabel = T, print_pdf = T, print_png = F, outputname) 
{
  if (is.null(w)) {
    w = 30 * colnum
  }
  if (is.null(h)) {
    h = ceiling(length(names(muts_basis))/colnum) * 4
  }
  ...
  if (length(cnames) > 1) {
    ...
    for (i in 1:dim(muts_basis2)[2]) {
      plottitle <- paste0(names(muts_basis2)[i], ", N =", 
                          sum(muts_basis2[, i]))
      if (print_Xlabel == T) {
        p <- x_plot476_single(data.frame(Sample = muts_basis2[, 
                                                              i], IndelType = rownames(muts_basis2)), text_size, 
                              plottitle)
      }
      else {
        p <- x_plot476_single(data.frame(Sample = muts_basis2[, 
    ...
    if (is.null(outputname) == F) {
      if (print_pdf) {
        ...
        grDevices::pdf(file = filename, onefile = TRUE, 
                       width = w, height = h)
    ...
    plottitle <- paste0(names(muts_basis2)[1], ", N =", sum(muts_basis2[, 
                                                                        1]))
    p_all <- list()
    if (print_Xlabel == T) {
      p <- x_plot476_single(data.frame(Sample = muts_basis2[, 
                                                            1], IndelType = rownames(muts_basis2)), text_size, 
                            plottitle)
    }
    else {
      p <- x_plot476_single(data.frame(Sample = muts_basis2[, 
...
x_plot476_single <- function (muts_basis, text_size=10, plot_title="test"){
  ...
  p <- ggplot2::ggplot(data = muts_basis_melt, ggplot2::aes(x = IndelType, 
                                                            y = freq, fill = Indel)) + 
    ggplot2::geom_bar(stat = "identity", position = "dodge", width = 0.7) + ggplot2::xlab(NULL) + 
    ggplot2::ylab("Count")
  ...
  p <- p + ggplot2::scale_fill_manual(values = indel_mypalette_fill_all) + 
    ggplot2::coord_cartesian(ylim = c(0, max(muts_basis_melt$freq) * 1.45), 
                             expand = FALSE)
  p <- p + ggplot2::theme_classic() + ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 90, 
                                                                                         vjust = 2, size = 8, colour = "black", hjust = 1), 
                                                     axis.text.y = ggplot2::element_text(size = 20, colour = "black"), 
  ...
  p <- p + ggplot2::geom_rect(data = blocks3, ggplot2::aes(xmin = xmin, 
    ...
    ggplot2::geom_text(data = blocks3, 
                       ggplot2::aes(x = (xmax + xmin)/2, y = (ymax + ymin)/2, 
                                    label = labels, colour = cl), size = 8, fontface = "bold", 
                       inherit.aes = F) + 
  ...
  p <- p + ggplot2::geom_rect(data = blocks, ggplot2::aes(xmin = xmin, 
                              ...
                              inherit.aes = F) + ggplot2::geom_text(data = blocks, 
                                                                    ggplot2::aes(x = (xmax + xmin)/2, y = (ymax + ymin)/2, 
                                                                                 label = labels, colour = cl), size = 6, fontface = "bold", 
                                                                    inherit.aes = F)
  return(p)
}
# 加载依赖
...
# 4️⃣ 批量绘图并保存 PNG
output_dir <- "/home/wuxueming/shinyapp/id476"
dir.create(output_dir, showWarnings = FALSE)

for (i in 1:ncol(df_matrix)) {
  # 提取每一列
  sample_name <- colnames(df_matrix)[i]
  sample_data <- data.frame(IndelType = rownames(df_matrix),
                            freq = df_matrix[, i])
  
  # 调用你的绘图函数
  p <- x_plot476_single(sample_data, plot_title = sample_name)
  
  # 保存为 PNG 文件
  outfile <- file.path(output_dir, paste0(sample_name, "_476all.png"))
  ggsave(filename = outfile, plot = p, width = 50, height = 6, dpi = 600,limitsize = FALSE)
