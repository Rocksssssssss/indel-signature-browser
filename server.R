id89_df <- read.delim(
  "./ID89_ID83_connection_example.txt",
  header = TRUE,
  ...
  if (!is.null(sig$id83_name) && !is.na(sig$id83_name)) {
    ...
    if (length(id83_groups[[id83_key]]$thumbnail) == 0) {
      ...
      if (file.exists(file.path("www/id83thumbnail", thumbnail_name))) {
        id83_groups[[id83_key]]$thumbnail <- file.path(
          "id83thumbnail",
          thumbnail_name
        )
      }
    ...
    if (length(matched89) > 0 && length(matched83) > 0) {
      ...
      showModal(modalDialog(
        ...
        tags$p(paste0(
          "The signature '",
          search_term,
          "' appears in both Classifications."
        )),
        ...
        if (
          ...
          div(
            ...
            div(
              ...
              tags$span(
                style = "font-size:16px; color:#27ae60; font-weight:700; 
      ...
      fluidRow(
        lapply(names(id83_groups), function(id83_name) {
          ...
          member_badges <- lapply(id83_info$members, function(m) {
            tags$span(
              m,
              style = "display:inline-block; background-color:#3498db !important; color:#ffffff !important; padding:3px 8px; border-radius:12px; font-size:11px; margin-right:4px; margin-bottom:4px; font-weight:600; box-shadow: 0 2px 4px rgba(52, 152, 219, 0.2);"
            )
          })
            ...
            div(
              ...
              actionLink(
                ...
                label = tagList(
                  h4(
                    id83_name,
                    style = "color:#27ae60 !important; margin-top:0; font-weight:700;"
                  ),
                  ...
                  if (
                    ...
                    tags$img(
                      src = thumbnail_path,
                      style = "width:100%; max-width:200px; height:auto; margin-bottom:15px; border-radius:8px; border:2px solid #e8eaed; cursor:pointer;"
                    )
                  } else {
                    div(
                      style = "height:120px; line-height:120px; color:#95a5a6; margin-bottom:15px; cursor:pointer;",
                      icon("image", style = "font-size:48px;"),
                      br(),
                      tags$small("No Thumbnail")
                    )
                  },

                  # 2. Members 区块
                  div(
                    style = "background:#ecf0f1 !important; padding:15px; border-radius:8px; margin-bottom:15px; text-align:left;",
                    ...
                    # 这里放回 members_text
                    div(
                      style = "font-size:12px; color:#34495e !important; font-weight:500; line-height:1.4;",
                      members_text
                    )
                  )
                ),
              ...
              fluidRow(
                ...
                if (
                  ...
                  column(
                    ...
                    div(
                      class = "img-label",
                      "Sample A in COSMIC83 representation"
                    ),
                    tags$img(
